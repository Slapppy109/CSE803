...
def projectLinesOrthogonal(f,R,t,L):
    #Given:
    #  Nx6 lines [x1,y1,z1,x2,y2,z2]
    #  3x3 rotation matrix R
    #  3x1 translation t
    ...
    for i in range(L.shape[0]):
        #rotate and translate
        p = np.dot(p_matrix, np.dot(R,L[i,:3])) + t
        pp = np.dot(p_matrix, np.dot(R,L[i,3:])) + t
        #apply projection u = x*f/z; v = y*f/z
        pL[i,:2] = sf * (p[0]*f/p[2]), sf * p[1]*f/p[2]
        pL[i,2:] = sf * pp[0]*f/pp[2], sf * pp[1]*f/pp[2]
    return np.vstack(pL)

def renderCube(f=1,scaleFToSize=None,t=(0,0,1),R=np.eye(3), O = False):
    #Given:
    #   f -- the focal length
    ...
    t = np.array(t)
    if O:
        pL = projectLinesOrthogonal(f,R,t,L)
    else:
